...
function statusBadge(status: string, lang: "zh" | "en") {
  ...
  return active ? (
    ...
    <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-500 ring-1 ring-inset ring-slate-500/10">
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-xs text-slate-400">{t("createSub")}</p>
        <form action="/projects/create" method="post" className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_1.2fr_auto]">
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-500">{t("org")}</span>
            <input
              name="org_id"
          ...
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-500">{t("name")}</span>
            <input
              name="name"
          ...
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-500">{t("slug")}</span>
            <input
              name="slug"
              placeholder="payment-service"
              required
          ...
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-500">{t("gitRemote")}</span>
            <input
              name="git_remote"
      ...
      {projects.length === 0 ? (
        <div className="mt-10 flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white/60 px-6 py-16 text-center">
          ...
          <h3 className="mt-4 text-sm font-semibold text-slate-900">{t("noProjects")}</h3>
          <p className="mt-1 max-w-sm text-sm text-slate-500">{t("noProjectsHint")}</p>
        </div>
      ) : (
        <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((project) => (
            <article
              key={project.id}
              className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-blue-200 hover:shadow"
            >
              <div className="flex items-start justify-between gap-3">
                <Link href={`/projects/${project.id}`} className="min-w-0">
                  <h2 className="truncate text-[15px] font-semibold text-slate-900 group-hover:text-blue-700">
                    {project.name}
                  </h2>
