...
def test_admin_cli_export_project_archive_writes_manifest_and_jsonl_assets(tmp_path):
    ...
    assert manifest["schema_revision"] == "20260902_0014"
...
def test_admin_cli_compatibility_check_reports_protocol_manifest(tmp_path):
    ...
    assert report["schema_revision"] == "20260902_0014"
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):
            if self.path == "/ready":
                ...
                self.end_headers()
                self.wfile.write(
                    b'{"status":"ready","checks":{"database":{"status":"ok"},"schema":{"revision":"20260902_0014"}}}'
                )
                return
