...
目标：验证 Agora 能接收真实 CI 流水线的质量信号，自动写入项目 WorkItem 的质量证据，并让 AI 工具和 Web 项目状态同步看到最新质量状态。
...
- Web UI 只负责查看结果：`Project status` 会显示 CI evidence、quality dimensions、delivery readiness 和 blockers。
